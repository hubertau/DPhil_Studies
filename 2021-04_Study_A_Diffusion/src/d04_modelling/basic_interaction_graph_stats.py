...
import sys
sys.path.insert(0, '../d07_visualisation/')
...
def main(args):
    ...
    G_connected_components = list(G_connected_components)
    density = nx.density(G)
    average_local_clustering_coeff = nx.average_clustering(G)
    weighted_average_local_clustering_coeff = nx.average_clustering(G, weight = 'weight')
    ...
    # subgraph_1_betweenness_centrality = nx.betweenness_centrality(S[0])
    weights = list(G.edges(data = True))
    weights = [i[1]['weight'] for i in weights]
    weight_cdf_values = generate_cdf_values(weights)
    f = plt.figure(figsize=(25,15))
    cdf_plot_save_file = '../../results/plot_rt_subgraph_1_weight_cdf.png'
    weight_cdf_values.plot(x = 'value', y = 'cdf', grid = True)
    ...
    print('Weighted Average Local Clustering Coefficient: {}'.format(weighted_average_local_clustering_coeff))
    ...
    print('Lengths of subgraphs: {}'.format([len(i) for i in S]))
    # print('Biggest subgraph betweenness centrality: {}'.format(subgraph_1_betweenness_centrality))

    # try drawing one of the connected components
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--overwrite',
        help = 'recreate the graph object.',
        action = 'store_true',
        default = False
    )
