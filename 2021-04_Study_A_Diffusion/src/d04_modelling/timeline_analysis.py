...
class TimelineAnalyzer(BSCresults):
    ...
    def _create_image_path_dir(self):

        # create images folder to store results
        self.image_path = os.path.join(self.bsc_dir, 'images/')
    ...
    def plot_FAS_activity(self, search_query_text_file):
        ...
        # self._plot_setup()
        # self._create_image_path_dir()
        self.FAS_image_path = os.path.join(os.path.split(self.data_dir)[0],'images')

        if os.path.isdir(self.FAS_image_path):
            pass
        else:
            os.mkdir(self.FAS_image_path)
            print('{} Directory created.'.format(self.FAS_image_path))
        ...
        # save plot
        plot_savename = os.path.join(self.FAS_image_path, 'FAS_activity.png')
